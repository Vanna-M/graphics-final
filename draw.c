...
void draw_polygons( struct matrix *polygons, screen s, zbuffer zb, color c ) {
	...
	for (point=0; point < polygons->lastcol-2; point+=3) {
		...
		if ( normal[2] > 0 ) {
			...
			double x0 = polygons->m[0][point];
			double y0 = polygons->m[1][point];
			double z0 = polygons->m[2][point];
			double x1 = polygons->m[0][point+1];
			double y1 = polygons->m[1][point+1];
			double z1 = polygons->m[2][point+1];
			double x2 = polygons->m[0][point+2];
			double y2 = polygons->m[1][point+2];
			double z2 = polygons->m[2][point+2];

			c.red = (point * 100)%255;
			c.green = (point * 233) % 255;
			c.blue = (point * 47) % 255;

			scanline(x0,y0,z0,x1,y1,z1,x2,y2,z2,zb,s,c);

		}
	}
}

void scanline(double x, double y1, double z, double x2, double y2, double z2, double x3, double y3, double z3,zbuffer zb,screen s,color c){

	double vals[9];
	//weirdly named to not conflict
	findVals(vals,x,y1,z,x2,y2,z2,x3,y3,z3);
	...
	//x0 = (tx-bx)/(ty-by)
	double dx0 = ty == by ? 0 : (tx - bx) / (ty - by);
	//z0 = (tz-bz)/(ty-by)
	double dz0 = ty == by ? 0 : (tz - bz) / (ty - by);

	//first half changes
	double dx1 = my == by ? 0 : (mx - bx) / (my - by);
	double dz1 = my == by ? 0 : (mz - bz) / (my - by);

	int y;

	for (y = by; y < my; y++){

		draw_line((int)x0,y,z0,(int)x1,y,z1,s,zb,c);
...
//returns in form: bx,by,bz,mx,my,mz,tx,ty,tz
void findVals(double vals[9], double x0, double y0, double z0, double x1, double y1, double z1, double x2, double y2, double z2){

	if (y0 < y1){
		//y0,y1,y2
		if (y1 < y2){
			double a[9] = {x0,y0,z0,x1,y1,z1,x2,y2,z2};
			memcpy(vals,a,9*sizeof(double));
		}
		//y0,y2,y1
		else if (y0 < y2){
			double a[9] = {x0,y0,z0,x2,y2,z2,x1,y1,z1};
			memcpy(vals,a,9*sizeof(double));
		}
		//y2,y0,y1
		else{
			double a[9] = {x2,y2,z2,x0,y0,z0,x1,y1,z1};
			memcpy(vals,a,9*sizeof(double));
		}
	}
		...
		//y1,y2,y0
		if (y2 < y0){
			double a[9] = {x1,y1,z1,x2,y2,z2,x0,y0,z0};
			memcpy(vals,a,9*sizeof(double));
		}
		//y1,y0,y2
		else{
			double a[9] = {x1,y1,z1,x0,y0,z0,x2,y2,z2};
			memcpy(vals,a,9*sizeof(double));
		}
	}
	...
	//y2,y1,y0
		...
		double a[9] = {x2,y2,z2,x1,y1,z1,x0,y0,z0};
		memcpy(vals,a,9*sizeof(double));
	}
